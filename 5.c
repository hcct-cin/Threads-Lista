...
double resultadoPorMatrizEsparsa[4][4];  // Variavel para armazenar o resultado da multiplicacao pela outra matriz esparsa
...
double aux2[4][4];                       // Variavel para representar a outra matriz esparsa por uma matriz densa
...
void preencheMatrizEsparsa(){
    ...
    MatrizEsparsa[3][1].elemento = 2.0;
                                ...
                                //  Nessa função eu preencho a matriz esparsa
void preencheMatrizEsparsa2(){
    // Linha 1
    OutraMatrizEsparsa[0][0].indiceNzero = 0;
    OutraMatrizEsparsa[0][0].elemento = 2.0;
    OutraMatrizEsparsa[0][1].indiceNzero = 1;
    OutraMatrizEsparsa[0][1].elemento = -1.0;
    // Linha 2
    OutraMatrizEsparsa[1][0].indiceNzero = 0;
    OutraMatrizEsparsa[1][0].elemento = -1.0;
    OutraMatrizEsparsa[1][1].indiceNzero = 1;
    OutraMatrizEsparsa[1][1].elemento = 2.0;
    OutraMatrizEsparsa[1][2].indiceNzero = 2;
    OutraMatrizEsparsa[1][2].elemento = -1.0;
    // Linha 3
    OutraMatrizEsparsa[2][0].indiceNzero = 1;
    OutraMatrizEsparsa[2][0].elemento = -1.0;
    OutraMatrizEsparsa[2][1].indiceNzero = 2;
    OutraMatrizEsparsa[2][1].elemento = 2.0;
    OutraMatrizEsparsa[2][2].indiceNzero = 3;
    OutraMatrizEsparsa[2][2].elemento = -1.0;
    // Linha 4
    OutraMatrizEsparsa[3][0].indiceNzero = 2;
    OutraMatrizEsparsa[3][0].elemento = -1.0;
    OutraMatrizEsparsa[3][1].indiceNzero = 3;
    OutraMatrizEsparsa[3][1].elemento = 2.0;
      ...
      for(j=0;j<4;j++){
        if(MatrizEsparsa[i][j].indiceNzero != 0 || MatrizEsparsa[i][j].elemento != 0.0){
          aux[i][MatrizEsparsa[i][j].indiceNzero] = MatrizEsparsa[i][j].elemento;}
                                ...
                                // Nessa funcao eu armazeno a matriz esparsa em uma matriz auxiliar2 na forma densa
void transformaEsparsa2(){  
    int i,j;
    
    for(i=0;i<4;i++){
      for(j=0;j<4;j++){
        if(OutraMatrizEsparsa[i][j].indiceNzero != 0 || OutraMatrizEsparsa[i][j].elemento != 0.0){
          aux2[i][OutraMatrizEsparsa[i][j].indiceNzero] = OutraMatrizEsparsa[i][j].elemento;}
        	 ...
        	 resultadoPorMatrizDensa[i][j] = 0;
       		 ...
       		 for(k=0;k< COLUNAS;k++) {
           		 resultadoPorMatrizDensa[i][j] = resultadoPorMatrizDensa[i][j] + aux[i][k]* MatrizDensa[k][j];
                                ...
                                // Funcao responsavel por multiplicar a matriz por outra matriz esparsa.
void *multMatrizEsparsa(void *tid){
    	int i,j, k;
        long  threadId = (*(long *)tid); 
	   ...
	   for(i=threadId; i < LINHAS; i = i + NUM_THREADS) {
      	...
      	for (j=0;j<COLUNAS;j++) {
        	 resultadoPorMatrizEsparsa[i][j] = 0;
  
       		 for(k=0;k< COLUNAS;k++) {
           		 resultadoPorMatrizEsparsa[i][j] = resultadoPorMatrizEsparsa[i][j] + aux[i][k]* aux2[k][j];
       		 }
      }
...
void multiplicaVetor(){
  ...
  for(t=0; t<NUM_THREADS; t++){
    ...
    printf("No Multiplica por Vetor : criando thread %ld\n", t+1);                       // Crio as N threads
...
void multiplicaMatrizDensa(){
  ...
  for(t=0; t<NUM_THREADS; t++){
    ...
    printf("No Multiplica por Matriz Densa : criando thread %ld\n", t+1);                       // Crio as N threads
    taskids[t] = (long *) malloc(sizeof(long)); *taskids[t] = t;
...
void multiplicaMatrizEsparsa(){

    pthread_t threads[NUM_THREADS];               // Declaro as threads 
  long *taskids[NUM_THREADS];                   // Identidade das threads
	int i,j,u; long t;   

    preencheMatrizEsparsa2();
    transformaEsparsa2();
	
  for(t=0; t<NUM_THREADS; t++){

    printf("No Multiplicar Matriz por Esparsa : criando thread %ld\n", t+1);                       // Crio as N threads
    taskids[t] = (long *) malloc(sizeof(long)); *taskids[t] = t;
	 	pthread_create(&threads[t],NULL,multMatrizEsparsa, (void *)taskids[t]);      // Mando as threads para a executar a rotina de threadCode.   
  }
  ...
  for(i=0; i < LINHAS; i++) {                                       // Printo o restultado
			 for (j=0;j<COLUNAS;j++) {
      		 printf("%.1lf\t", resultadoPorMatrizEsparsa[i][j]);
  		 }
